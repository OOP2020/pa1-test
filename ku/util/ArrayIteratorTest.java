...
public class ArrayIteratorTest {
	...
	public void testSingletonArray() {
		...
		Iterator<String> iter = new ArrayIterator<String>(array);
		assertTrue( iter.hasNext() );
		assertSame( array[0], iter.next() );
	...
	public void basicIteratorTest() {
		Object [] array = new Object[] {"one", new Integer(2), "three" };
		// copy the array to verify the ArrayIterator doesn't modify it
	...
	public void testArrayOfNulls() {
		array = new Object[] { null, null, null, null };
		iter = new ArrayIterator(array);
		assertFalse( iter.hasNext() );
		assertFalse( iter.hasNext() );
		...
		// calling next() should throw exception
		try {
			iter.next();
	...
	public void testSkipNulls() {
		array = new Object[] { null, "first", null, null, null, "last" };
		iter = new ArrayIterator(array);
		assertTrue( iter.hasNext() );
		...
		assertFalse( iter.hasNext() );
		// same test without calling hasNext
		array = new Object[] { null, "first", null, null, null, "last" };
		iter = new ArrayIterator(array);
		assertEquals( "first", iter.next() );
		assertEquals( "last", iter.next() );
	 ...
	 */
	//@Test  // this method is not part of the assignment. Skip the test.
